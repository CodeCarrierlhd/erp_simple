			...
			if (!err) {
				that.$axios.post('/login', {
					...
					if (res.code === 200) {
						...
						that.props.setUserInfo(Object.assign({}, res.result, { role: { type: res.result.type, name: res.result.name } }));
						localStorage.setItem('userInfo', JSON.stringify(Object.assign({}, res.result, { role: { type: res.result.type, name: res.result.name } })));
